<template>
  <div class="container my-5">
    <div class="card shadow-lg p-4 rounded-4">
      <h1 class="text-center text-primary mb-4">🧮 Calculadora Básica con defineComponent</h1>
        ...
        <fieldset class="mb-4">
          ...
          <div class="d-flex flex-wrap gap-3">
            <div class="form-check">
              <input class="form-check-input" type="radio" name="operador" id="sumar" value="opcionSumar"
                v-model="operador">
              <label class="form-check-label" for="sumar">➕ Sumar</label>
            </div>

            <div class="form-check">
              <input class="form-check-input" type="radio" name="operador" id="restar" value="opcionRestar"
                v-model="operador">
              <label class="form-check-label" for="restar">➖ Restar</label>
            </div>

            <div class="form-check">
              <input class="form-check-input" type="radio" name="operador" id="multiplicar" value="opcionMultiplicar"
                v-model="operador">
              <label class="form-check-label" for="multiplicar">✖️ Multiplicar</label>
            </div>

            <div class="form-check">
              <input class="form-check-input" type="radio" name="operador" id="dividir" value="opcionDividir"
                v-model="operador">
        ...
        <div class="mb-3 row">
          ...
          <div class="col-sm-9">
            <input type="text" class="form-control form-control-lg text-center fw-bold"
              :class="{ 'text-danger': resultado === 'Error' }" id="resultado" :value="resultado" readonly>
          </div>
        </div>
      </form>
    </div>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const numero1 = ref(props.numero1 ?? 5);
    const numero2 = ref(props.numero2 ?? 10);

    const operador = ref('opcionSumar');
